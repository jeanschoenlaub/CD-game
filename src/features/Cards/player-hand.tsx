...
export function PlayerHand({ cards }: PlayerHandProps) {
    ...
    */
    ...
    /*
    * Card Geometry with default texture over-ridden in map below, texture handled at draw
    */
    ...
    * Also handles the hovered animation 
    */
    useFrame(() => {
        ...
        <>   
        {
            cards.map((card, index) => (
                <mesh
                    ...
                    material={new THREE.MeshBasicMaterial({map: card.texture, color:0xEEEEEE})}
                    ref={cardRefs.current[index]}
                    // Initial position and rotation are not critical as they will be overridden
                    position={[0, 0, 0]} 
                    rotation={[0, 0, 0]}
                    onPointerDown={(event) => handleCardSelect(card.id,event, cardRefs.current[index])}
                    onPointerUp={() => handleCardDrop(card.id, cardRefs.current[index])}
                    //PERF these could be quite CPU intensive as testing each frame
                    onPointerEnter={(event) => handleCardHoverIn(card.id,event, cardRefs.current[index])}
                    onPointerLeave={(event) => handleCardHoverOut(card.id, event, cardRefs.current[index])}
                />
            ))}
        </>
    );
